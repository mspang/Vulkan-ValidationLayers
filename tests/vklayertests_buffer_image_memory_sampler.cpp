...
TEST_F(VkLayerTest, SamplerImageViewFormatUnsupportedFilter) {
    ...
    for (uint32_t i = VK_FORMAT_R4G4_UNORM_PACK8; i <= VK_FORMAT_ASTC_12x12_SRGB_BLOCK; i++) {
        ...
        if (nonlinear_format == VK_FORMAT_UNDEFINED && props.linearTilingFeatures != 0 && !(props.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
            nonlinear_format = static_cast<VkFormat>(i);
        } else if (nonlinear_format == VK_FORMAT_UNDEFINED && props.optimalTilingFeatures != 0 && !(props.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
            nonlinear_format = static_cast<VkFormat>(i);
            nonlinear_format_tiling = VK_IMAGE_TILING_OPTIMAL;
        }

        if (noncubic_format == VK_FORMAT_UNDEFINED && props.linearTilingFeatures != 0 && !(props.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_CUBIC_BIT_EXT)) {
            noncubic_format = static_cast<VkFormat>(i);
        } else if (noncubic_format == VK_FORMAT_UNDEFINED && props.optimalTilingFeatures != 0 && !(props.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_CUBIC_BIT_EXT)) {
            noncubic_format = static_cast<VkFormat>(i);
