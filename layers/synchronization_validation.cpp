...
inline VkDeviceSize GetRealWholeSize(VkDeviceSize size, VkDeviceSize whole_size) {
...
void SyncValidator::ApplyBufferBarriers(AccessContext *context, VkPipelineStageFlags src_exec_scope,
    ...
    for (uint32_t index = 0; index < barrier_count; index++) {
        ...
        if (!buffer) continue;
        barrier.size = GetRealWholeSize(barrier.size, buffer->createInfo.size);
    ...
    for (uint32_t region = 0; region < regionCount; region++) {
        ...
        if (src_buffer) {
            ResourceAccessRange src_range =
                MakeRange(copy_region.srcOffset, GetRealWholeSize(copy_region.size, src_buffer->createInfo.size));
        ...
        if (dst_buffer && !skip) {
            ResourceAccessRange dst_range =
                MakeRange(copy_region.dstOffset, GetRealWholeSize(copy_region.size, dst_buffer->createInfo.size));
